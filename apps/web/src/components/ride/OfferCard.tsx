...
export function OfferCard({ offer, index, onAccept, onReject, loading }: OfferCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-[#111118] border border-[rgba(255,255,255,0.08)] rounded-2xl p-4 hover:border-[rgba(0,229,168,0.3)] transition-colors"
    >
      <div className="flex items-center gap-3 mb-4">
        {/* Avatar */}
        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#00E5A8] to-[#00D4AA] flex items-center justify-center font-bold text-white flex-shrink-0">
          {d?.avatarUrl ? (
        ...
        <div className="text-right">
          <div className="text-xl font-black font-mono bg-gradient-to-r from-[#00E5A8] to-[#00D4AA] bg-clip-text text-transparent">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex gap-2 ml-auto">
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-[#00E5A8] to-[#00D4AA] font-semibold text-sm hover:opacity-90 transition-opacity disabled:opacity-50"
          >
...
function getColorDot(color: string): string {
  const map: Record<string, string> = {
    ...
    azul: '#00E5A8', verde: '#2ED573', amarillo: '#FFD700', naranja: '#FFA502',
  };
