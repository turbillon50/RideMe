...
export default function OffersPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F2F6FB] flex flex-col">
      {/* Header */}
      <div className="safe-top px-4 pt-4 pb-3 border-b border-[rgba(13,27,61,0.06)]">
        <div className="flex items-center justify-between mb-3">
          <h1 className="font-black text-xl">Ofertas de choferes</h1>
          <button onClick={handleCancel} className="text-[#4A5876] hover:text-[#FF4757] transition-colors">
            <X size={22} />
          </button>
        </div>
        ...
        {/* Route summary */}
        {activeRide && (
          <div className="bg-[#FFFFFF] border border-[rgba(13,27,61,0.06)] rounded-2xl px-4 py-3 space-y-1.5">
            <div className="flex items-center gap-2 text-sm">
              <div className="w-2 h-2 rounded-full bg-[#2563EB]" />
              <span className="text-[#4A5876] truncate">{activeRide.origin_address ?? 'Origen'}</span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <MapPin size={10} className="text-[#00B4FF]" />
              <span className="text-[#4A5876] truncate">{activeRide.destination_address ?? 'Destino'}</span>
            </div>
            <div className="flex items-center justify-between pt-1">
              <span className="text-xs text-[#4A5876]">Tu precio</span>
              <span className="font-mono font-bold text-[#2563EB]">${activeRide.proposed_price}</span>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Timer */}
      <div className="flex items-center justify-center gap-2 py-3 bg-[rgba(37,99,235,0.06)] border-b border-[rgba(255,255,255,0.04)]">
        <Clock size={14} className={timeLeft < 60 ? 'text-[#FF4757]' : 'text-[#4A5876]'} />
        <span className={`text-sm font-mono font-bold ${timeLeft < 60 ? 'text-[#FF4757]' : 'text-[#4A5876]'}`}>
        ...
        <AnimatePresence>
          {offers.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center h-48 text-center">
              ...
              <p className="text-[#4A5876]">Buscando choferes cerca de ti...</p>
              <p className="text-xs text-[#8693AB] mt-1">Las ofertas aparecerán aquí</p>
            </motion.div>
          ) : (
            offers.map((offer, i) => (
              <OfferCard
